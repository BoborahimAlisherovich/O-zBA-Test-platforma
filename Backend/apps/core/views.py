...
def _finalize_attempt(attempt, submitted_answers=None, submitted_time_taken=None):
    ...
    correct = 0
    for q_payload in question_payload:
        qid = str(q_payload.get("id"))
        chosen = answers.get(qid)
        correct_idx_in_payload = q_payload.get("correct_index")
        
        if correct_idx_in_payload is None:
            question = question_map.get(qid)
            if question:
                correct_idx_in_payload = question.correct_index
                
        if chosen is not None and correct_idx_in_payload is not None and int(chosen) == correct_idx_in_payload:
            correct += 1
